...
public class RepartidorController {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<?> actualizar(@PathVariable Long id, @RequestBody Repartidor r) {
        try {
    ...
    /** Asignar siguiente repartidor (dequeue) **/
    @GetMapping("/asignar")
    public ResponseEntity<?> asignarSiguiente() {
        Repartidor r = svc.dequeue();
        if (r == null) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("No hay repartidores disponibles");
        }
        return ResponseEntity.ok(r);
    }
}
